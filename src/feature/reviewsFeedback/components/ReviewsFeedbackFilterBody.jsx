...
const ReviewsFeedbackFilterBody = ({ filterData, onFilterChange }) => {
  const months = [
  ...
  const handleRatingChange = (rating) => {
    onFilterChange({ ...filterData, rating: rating.toString() });
  };
  ...
  return (
    <Box>
      {/* Rating Section - Radio Buttons */}
      <FilterSection title="Rating">
        {ratings.map((rating) => (
          <RadioOption
            key={rating}
            label={rating.toString()}
            checked={filterData.rating === rating.toString()}
      ...
      <FilterSection title="Product name">
        <Box
          ...
          onChange={(e) => onFilterChange({ ...filterData, productName: e.target.value })}
          sx={{
            width: '100%',
            py: '0.75rem',
            px: '1rem',
...
export default ReviewsFeedbackFilterBody;
